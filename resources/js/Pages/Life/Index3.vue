        ...
        <div class="dis">
            <div class="bor mr20">
                <table>
                    ...
                    <tr>
                        <td>現在のターン</td>
                        <td>{{ isPlayerTune ? characters[0].name : characters[1].name }}</td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td>{{ isPlayerTune ? characters[0].name : characters[1].name }}の出た目</td>
                        <td>{{ dice }}</td>
                    </tr>
                </table>
                <button v-if="isPlayerTune && !isGameOver" @click="rollDice()" class="btn btn-primary">{{ characters[0].name }}のターン サイコロを振る</button>
                <button v-if="!isPlayerTune && !isGameOver" @click="cpuRollDice()" class="btn btn-secondary">{{ characters[1].name }}のターン サイコロを振る</button>
            </div>

            <div class="dis">
                <div v-for="(character, index) in characters" :key="index" class="bor wi200 mr20">
                    <p>{{ character.name }}</p>
                    <p>円</p>
                    <p>次の行動</p>
                    <button v-if="isPlayerTune && index == 0 && !isGameOver" @click="rollDice()" class="btn btn-primary">{{ characters[0].name }}のターン<br>サイコロを振る</button>
                <button v-if="!isPlayerTune && index == 1 && !isGameOver" @click="cpuRollDice()" class="btn btn-secondary">{{ characters[1].name }}のターン<br>サイコロを振る</button>
                </div>
            </div>
        </div>
    </div>
...
// const imgs = [
//     { img: "storage/img/1.jpg", status: true },
//     { img: "storage/img/4.jpg", status: false },
//     { img: "storage/img/5.jpg", status: false },
// ];
...
export default {
    data() {
        return {
            ...
            isGameOver: false,
            characters: [
                { name: "てつろー", img: "", position: 1 },
                { name: "えみ", img: "", position: 1 },
            ],
            ...
            nameDisplay: false,
            imgs: [
                { img: "storage/img/1.jpg", status: false },
                { img: "storage/img/4.jpg", status: false },
                { img: "storage/img/5.jpg", status: false },
            ],
            error1: '',
        ...
        event(position) {
            for (const event of events) {
                if (position == event.cell) {
                    // const object = events.find(ob => ob.cell == position);
